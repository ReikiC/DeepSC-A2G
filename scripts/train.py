# -*- coding: utf-8 -*-
"""Train DeepSC (semantic encoder/decoder + channel codec + MINE).

Examples:
    uv run python scripts/train.py --channel Rayleigh
...
from deepsc.data import EurDataset, collate_data
from deepsc.models import DeepSC, Mine, initNetParams
from deepsc.signal import SNR_to_noise
from deepsc.training import train_step, val_step, train_mi
...
def train(epoch, args, net, optimizer, criterion, pad_idx, mi_net=None, mi_opt=None):
...
if __name__ == '__main__':
    ...
    for epoch in range(args.epochs):
        train(epoch, args, deepsc, optimizer, criterion, pad_idx, mi_net, mi_opt)
        avg_acc = validate(epoch, args, deepsc, pad_idx, criterion)
